...
func validationAddStaff(bodyIN io.ReadCloser) (changes.ArgAddStaff, error) {
	post := changes.ArgAddStaff{}
...
func (c *Changes) AddStaff(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	post, err := validationAddStaff(r.Body)
	if err != nil {
		errorcode.WriteError(errorcode.CodeDataInvalid, err.Error(), w)
		return
	}

	if err = c.op.AddStaff(post); err != nil {
